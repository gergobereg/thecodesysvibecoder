...
The variable normally lasts only for the current PowerShell window. To retain
the key for the current Windows user after reboot, optionally store it as a
...
In a newly opened PowerShell window, load and check it with:
...
### 13. Normal startup after a reboot

Start components in this order:

1. On Ubuntu, verify Ollama with `systemctl status ollama`.
2. On Ubuntu, verify AnythingLLM with `docker ps`.
3. On Windows, open the correct project in CODESYS.
4. Run `ide_scripts\run_in_ide_agent.py` inside that CODESYS instance.
5. Run the local launcher `inspect` command and verify the project path.
6. Start `Start-CodesysMcpBridge.ps1` with the API key and permission flags.
7. From Ubuntu, call `/health`.
8. Open AnythingLLM and start a new agent chat if tool definitions changed.

The MCP bridge does not start CODESYS and does not start the in-IDE agent.
